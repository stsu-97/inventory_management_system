...
export default function StaffNoteForm() {
  ...
  const [activeTab, setActiveTab] = useState('manual'); // 'manual', 'excel', 'ocr'
  ...
  const parseOCRText = (text) => {
    ...
    // Try to find patterns like "Product: Laptop Qty: 5 Buyer: John"
    lines.forEach(line => {
    ...
    // If no structured data found, try to parse as comma-separated
    if (parsed.length === 0) {
      lines.forEach(line => {
